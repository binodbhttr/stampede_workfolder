...
simdir = '/scratch/projects/xsede/GalaxiesOnFIRE/mhdcv/m12i_res7100_mhdcv/1Myr/sf-fire3-alpha01'
...
path="./sf-fire3-alpha01data_pkl/" #creating a path to store the data only if it does not exist
...
cluster_path="./sf-fire3-alpha01data_pkl/"
cluster_file_name="sf-fire3-alpha01_clusters_"+simname+"_snapshot_"+str(snapshot_start) 
...
for i in range(total_snaps):               #we run a for loop until the end of all snapshots
  ...
  for j in range(total_clusters):
    ...
    tracked_data={"ind_tracked":ind_tracked,"age_tracked":age_tracked,"iron_tracked":iron_tracked,"x_tracked":x_tracked,"y_tracked":y_tracked,"z_tracked":z_tracked,
  ...
  file_name="sf-fire3-alpha01_clusters_at_snapshot_"+str(snap)+".pkl"
  with open(path+file_name, 'wb') as output:
...
with open(path+"total_data_sf-fire3-alpha01_all_snapshots.pkl", 'wb') as output:

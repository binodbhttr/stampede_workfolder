...
simtype="fire2"
snapshot_start=596
snapshot_end=693 #ran out of memory after 646
...
for i in range(total_snaps):               #we run a for loop until the end of all snapshots
  ...
  cbar_ax1 = fig1.add_axes([0.07, 0.185, 0.04, 0.65]) # position of gray colorbar (left, bottom, width, height)
  ...
  time=(snap-snapshot_start+3)/1000
  keep_old=np.where((age <= time) & (abs(z) < 1.5))
  x0_old=x[keep_old]
  y0_old=y[keep_old]
  age_old=age[keep_old]
  
  age_old=1000*age_old #Converting to Myr
  cb2=ax.scatter(x0_old,y0_old,c=age_old,s=3,alpha=9,cmap=plt.cm.get_cmap('seismic'), vmin=np.min(age_old),vmax=100)
  cbar_ax = fig1.add_axes([0.83, 0.185, 0.03, 0.65]) # position of the colorbar (left, bottom, width, height)
  ...
  for j in range(total_clusters):
    ...
    ax.scatter(x_cluster,y_cluster,c=colors[j],s=90,marker="*",edgecolor='black')
    cluster_count+=1
    
  ax.set_xlim(-25,25)
  ax.set_ylim(-25,25)
  #ax.legend(bbox_to_anchor=(1,0.5), loc='center left')
  ax.set_title("Age of the Clusters= "+str(snap-snapshot_start)+" Myr")
  fig1.savefig(young_star_plotpath+str(snap)+"test_seismic.png",bbox_inches='tight',dpi=200)  
  fig1.clf()
  plt.close()  
  ################################################################
  #Now lets write our tracked data from each snapshots to a file
  new_stars.update({"x_new_stars":x0_old,"y_new_stars":y0_old,"age":age_old}) #access it using tracked_data_all_clusters[clusterid]["key"]

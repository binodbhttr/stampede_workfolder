...
cluster_group="_snapshot_596" #Remember to change it if you  are changing the star cluster you are tracking in given snapshot

path="./" #creating a path to store the data only if it does not exist
...
cluster_path="./"
cluster_file_name="clusters_"+simname+cluster_group 
...
for i in range(total_snaps):               #we run a for loop until the end of all snapshots
  ...
  file_name="chem_clusters_at_snapshot_"+str(snap)+".pkl"
  with open(path+file_name, 'wb') as output:

...
for s in range(snapnumber,696,3):
    part = gizmo.io.Read.read_snapshots(['all'],'snapshot_index', snapnumber,
                                        simulation_name=simname,
                                        simulation_directory=simdir, assign_hosts_rotation=True,
                                        assign_hosts=True)  
    ...
    ind, xcm, ycm, zcm, mtot, grpid, r90, r50, rmax =fof.find(x0,y0,z0, b=linking_length, 
                                                              mass=mass0, ncut=ncut)
    ngroup = len(mtot)
    ...
    for grp_index in range(ngroup):  #iterate over each group
        ...
        print('%s     %i     %.4f     %.4f    %.4f     %.2e     %.1f ' % (grpid[grp_index],nstar,xcm[grp_index], 
                                                                          ycm[grp_index], zcm[grp_index], mtot[grp_index],
                                                                          1000*rmax[grp_index]))
    ...
    path=work_data_path+"fire2_m12i_clusterdata_pkl_b4n5_rxyz_20/" #creating a path to store the data only if it does not exist

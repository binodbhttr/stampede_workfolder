...
datapath="./fire3data_pkl/"  #this is teh path where the data of our tracked clusters is
...
snapshot_start=669  #snapshot to begin creating the figure
...
total_clusters=19   #set it manually as we have not imported the data yet
...
##############################
##############################
plot_path="./plots/fire3_plots_with_gas/" #creating a path to store the plots only if it does not exist
...
for i in range(len(time)):                              
    ...
    fig1.savefig(plot_path+"fire3_clusters_snap"+str(plot_count+snapshot_start)+".png",bbox_inches='tight',dpi=200)
    plot_count+=1
    fig1.clf()
    plt.close()

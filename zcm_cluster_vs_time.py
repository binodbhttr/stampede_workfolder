...
ax1.set_ylabel('Z position of the CM of the stars in the clusters in parsec')
...
for i in range(len(cluster_groupid)):
    ...
    for j in range(n):
        ...
        snapshot_count+=1
    z_cm=z_cm_temp[1:len(z_cm_temp)]*1000 #converted into parsec
    #print(avg_r_cm)
    slope, intercept = np.polyfit(time,z_cm, 1)
    ax1.plot(time,z_cm,label=cluster_groupid[cluster_count]+",n_star="+str(n_star))
    #ax1.plot(time,slope*time+intercept,label=cluster_groupid[cluster_count]+"fitted")
    ax1.legend(loc='upper left')
    cluster_count+=1
plt.tight_layout()
fig1.savefig(plot_path+"zcm_all_clusters.png",dpi=150)
#########################################################
#########################################################
...
snap=690                #this is the snapshot at which the clusters were taken from using the fof algorithm
...
colors=['cyan','blue','green','magenta','yellow','teal','brown','tan','lime','red','orange','purple','grey','pink','navy','lawngreen','deepskyblue'] #17 colors
...
for i in range(len(time)):                              
    ...
    for j in range(total_clusters):  #total_clusters tells us how many clusters to look for
        ...
        cluster_mass=np.sum(importdata[time[i]+snapshot_start][cluster_count+1]["mass_tracked"])

...
simdir = '/scratch/projects/xsede/GalaxiesOnFIRE/mhdcv/m12i_res7100_mhdcv/1Myr/sf-fire3-alpha03'
...
path="./sf-fire3-alpha03data_pkl/" #creating a path to store the data only if it does not exist
...
cluster_path="./sf-fire3-alpha03data_pkl/"
cluster_file_name="sf-fire3-alpha03_clusters_"+simname+"_snapshot_"+str(snapshot_start) 
...
for i in range(total_snaps):               #we run a for loop until the end of all snapshots
  ...
  file_name="sf-fire3-alpha03_clusters_at_snapshot_"+str(snap)+".pkl"
  with open(path+file_name, 'wb') as output:
...
with open(path+"total_data_sf-fire3-alpha03_all_snapshots.pkl", 'wb') as output:
